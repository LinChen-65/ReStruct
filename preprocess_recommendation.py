...
def preprocess_douban_movie(load_prefix, save_prefix, threshold, seed):
    um = pd.read_csv(os.path.join(load_prefix, "user_movie.dat"), encoding='utf-8', delimiter='\t', names=['uid', 'mid', 'rating']).drop_duplicates(subset=['uid', 'mid']).reset_index(drop=True)
    uu = pd.read_csv(os.path.join(load_prefix, "user_user.dat"), encoding='utf-8', delimiter='\t', names=['u1', 'u2', 'weight']).drop_duplicates(subset=['u1', 'u2']).reset_index(drop=True) ## not full; asym
    ug = pd.read_csv(os.path.join(load_prefix, "user_group.dat"), encoding='utf-8', delimiter='\t', names=['uid', 'gid', 'weight']).drop_duplicates(subset=['uid', 'gid']).reset_index(drop=True) ## user not full
    ma = pd.read_csv(os.path.join(load_prefix, "movie_actor.dat"), encoding='utf-8', delimiter='\t', names=['mid', 'aid', 'weight']).drop_duplicates(subset=['mid', 'aid']).reset_index(drop=True) ## 33572; movie not full
    md = pd.read_csv(os.path.join(load_prefix, "movie_director.dat"), encoding='utf-8', delimiter='\t', names=['mid', 'did', 'weight']).drop_duplicates(subset=['mid', 'did']).reset_index(drop=True) ## movie not full
    mt = pd.read_csv(os.path.join(load_prefix, "movie_type.dat"), encoding='utf-8', delimiter='\t', names=['mid', 'tid', 'weight']).drop_duplicates(subset=['mid', 'tid']).reset_index(drop=True) ## movie not full
    ...
    if not os.path.exists(os.path.join(save_prefix, "unconnected_pairs_offset.npy")):
        ...
        np.save(os.path.join(save_prefix, "unconnected_pairs_offset"), np.array(unconnected_pairs_offset))
        print('unconnected_pairs_offset generated. Used time: ', time.time()-start)
    ...
    #* positive pairs
    um_pos = um[um['rating'] > threshold].to_numpy()[:, :2] - 1

    #! negative rating
    neg_ratings = um[um['rating'] < threshold+1].to_numpy()[:, :2] - 1
    ...
    np.savez(os.path.join(save_prefix, f"pos_pairs_offset_larger_than_{threshold}_{seed}"), train=um_pos_train, val=um_pos_val, test=um_pos_test)
...
if __name__ == '__main__':
    load_prefix = './HIN-Datasets-for-Recommendation-and-Network-Embedding-master'
    save_prefix = './data_recommendation'

    dataset = sys.argv[1]
    load_prefix = os.path.join(load_prefix, dataset)
    save_prefix = os.path.join(save_prefix, dataset)
    ...
    if dataset == "Yelp":
        preprocess_yelp(load_prefix, save_prefix, threshold, seed)
    elif dataset == "Douban_Movie":
        preprocess_douban_movie(load_prefix, save_prefix, threshold, seed)
    elif dataset == "Amazon":
        preprocess_amazon(load_prefix, save_prefix, threshold, seed)
