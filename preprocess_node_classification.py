...
def main(dataset):
    prefix_to_get = os.path.join("./data_node_classification/", dataset)
    prefix_to_save = os.path.join("./data_node_classification/", dataset)
    with open(os.path.join(prefix_to_get, "edges.pkl"), "rb") as f:
        edges = pkl.load(f)
        f.close()
    ...
    np.save(os.path.join(prefix_to_save, "node_types"), node_types)
